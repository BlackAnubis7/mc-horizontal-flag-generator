...
try:
    ...
    countries_imported = True
except ModuleNotFoundError:
    print('Could not import countries_dict.py file. Remember to have it in the same directory as the main file')
    countries_imported = False

TAG = 'flagstand'
DFI = math.atan(0.26/0.495)
R = math.sqrt(0.26**2+0.495**2)
...
def error(mess):
    print(mess + ' Press ENTER to exit.')
    input()
    raise Exception(mess)


#/setblock 36 72 1174 minecraft:red_wall_banner[facing=north]{Patterns:[{Pattern:"ss",Color:0},{Pattern:"tr",Color:11}]}  (RAW F3+I)
#/setblock 21 67 1165 minecraft:red_banner[rotation=12]{Patterns:[{Pattern:"ss",Color:0},{Pattern:"tr",Color:11}]}  (RAW F3+I)
#minecraft:red_banner{BlockEntityTag:{Patterns:[{Pattern:ss,Color:0},{Pattern:bl,Color:11}]}}  (Target input)
...
for arg in args:
    if arg in 'nneennwwsseessww' and len(arg) in (1, 2, 3):
        ...
        try:
            ...
            print('Argument "{}" not recognised. Program will ignore it.'.format(arg))
# print(rot, side, fpv, fph)  # print used settings (manual debug tool)
...
if countries_imported and flag.upper() in COUNTRIES:
    flag = COUNTRIES[flag.upper()]
elif len(flag) == 2 and flag.isalpha():
    if countries_imported:
        error('Country code not found')
    else:
        error('Unable to recognise country code - the list was not imported')
elif 'banner' not in flag:  # NotBanner exception
    error('The input was not of a banner item')
...
if m is not None:
    flag = flag.replace(m.group(0), '&')  # '&' is making it easier to find the right '['
    flag = flag.replace('[&', '')
...
if 'c' in side:
    ...
    print('/item replace entity @e[tag={}_cw,limit=1,{},dx=0.2,dy=0.2,dz=0.2] armor.head with {}}}'.format(TAG, xyz_find, flip(flag, 'vh')))
if 'a' in side:
    ...
    print('/item replace entity @e[tag={}_acw,limit=1,{},dx=0.2,dy=0.2,dz=0.2] armor.head with {}}}'.format(TAG, xyz_find, flip(flag, 'h')))
input()
